...
+
+
+def perform_clustering_analysis(*args, **kwargs):
+    raise NotImplementedError("Clustering analysis has been removed.")
+
+
+def bootstrap_evaluation(*args, **kwargs):
+    raise NotImplementedError("Bootstrap evaluation has been removed.")
+
+
+def print_bootstrap_results(*args, **kwargs):
+    return
+
+
...
+    icd_source_df=None,
+):
+    """Construct binary labels for evaluation using only survival_df.
+
+    Logic:
+      - ICD==1: use appropriate_icd_shock_col
+      - ICD==0: use death_col
...
+        cols = [id_col, icd_indicator_col, appropriate_icd_shock_col, death_col]
...
+        merge_cols = [id_col]
+        if appropriate_icd_shock_col in survival_unique.columns:
+            merge_cols.append(appropriate_icd_shock_col)
...
+    shock = pd.to_numeric(out.get(appropriate_icd_shock_col), errors="coerce").fillna(0).astype(int)
+    death = pd.to_numeric(out.get(death_col), errors="coerce").fillna(0).astype(int)
+    derived = np.where(icd_values == 1, shock, death)
+    out[output_label_col] = derived.astype(int)
...
+    evaluate using survival_df-derived labels for all samples (ICD→shock, No-ICD→death).
+
+    Requirements:
...
+      - survival_df: contains id_col, appropriate_icd_shock_col, and death_col
+
+    Notes:
+      - survival_label_col is the derived binary label from survival_df based on ICD indicator:
+        ICD==1 → appropriate_icd_shock_col; ICD==0 → death_col. Used only for evaluation.
+
+    Returns:
...
+
+    # Derive validation labels using ICD from infer_df and death for no-ICD
...
+    evaluate by ICD vs no-ICD groups using survival_df-derived labels (ICD→shock, No-ICD→death).
+
+    For evaluation:
+      - ICD group: ground truth = appropriate ICD shock from survival_df (by MRN/id join)
+      - No-ICD group: ground truth = death from survival_df (by MRN/id join)
...
+      - If a group's rows do not have survival-derived labels (e.g., missing joins),
...
+    icd_source_df=None,
+):
+    """Construct binary labels for evaluation using only survival_df (ICD→shock, No-ICD→death)."""
+    survival_unique = survival_df.drop_duplicates(subset=[id_col], keep='first')
+
+    if icd_source_df is None:
+        cols = [id_col, icd_indicator_col, appropriate_icd_shock_col, death_col]
...
+
+    - Uses survival-derived labels in survival_label_col
...
+    )
+
+    # Also compute metrics by Sex×Pred groups using survival-derived labels
...
+        print("\n===== Metrics by Sex × Predicted risk (using survival-derived labels) =====")
...
+      - ICD==0 -> compare to survival_df[death_col]
+
+    Returns:
+      - eval_metrics: dict with overall/male/female metrics computed on survival-derived labels
